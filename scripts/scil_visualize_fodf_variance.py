...
def _build_arg_parser():
    ...
    p.add_argument('--var_opacity', default=0.1, type=float)
...
def _get_data_from_inputs(args):
    ...
    if args.peaks:
        ...
        if args.peaks_values:
            ...
            if peak_vals.shape[:3] != shape[:-1]:
                raise ValueError('Peaks volume dimensions {0} do not agree '
                                 'with fODF dimensions {1}.'
                                 .format(peak_vals.shape, shape))
            data['peaks_values'] =\
